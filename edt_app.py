...
# =============================================================================
# BLOC PRINCIPAL UNIQUE (remplace tout depuis la ligne 3388)
# =============================================================================
...
if not st.session_state["user_data"]:
    ...
    with t_conn:
        email_input = st.text_input("Adresse Email", key="login_email_v1")
        pass_input = st.text_input("Mot de passe", type="password", key="login_pass_v1")
        if st.button("Se connecter au portail", use_container_width=True, key="btn_login_v1"):
    ...
    with t_ins:
        ...
        new_nom = st.selectbox("Sélectionnez votre nom (tel qu'il apparaît dans l'EDT)", noms_possibles, key="ins_nom_v1")
        new_email = st.text_input("Votre adresse Email", key="ins_email_v1")
        new_pass = st.text_input("Choisissez un mot de passe", type="password", key="ins_pass_v1")
        confirm_pass = st.text_input("Confirmez le mot de passe", type="password", key="ins_confirm_v1")
        if st.button("Créer mon compte", use_container_width=True, key="btn_ins_v1"):
            if not new_email or not new_pass:
                st.warning("Veuillez remplir tous les champs.")
            elif new_pass != confirm_pass:
                st.error("Les mots de passe ne correspondent pas.")
    ...
    with t_adm:
        code_admin = st.text_input("Code de sécurité Administration", type="password", key="admin_code_v1")
        if st.button("Accès Administration", use_container_width=True, key="btn_admin_v1"):
...
with st.sidebar:
    ...
    portail = st.selectbox("🚀 Sélectionner Espace", options_portail, key="sel_portail_v1")
    st.divider()
    ...
    if portail == "📖 Emploi du Temps" and is_admin:
        ...
        poste_sup = st.checkbox("Poste Supérieur (Décharge 3h)", key="poste_sup_v1")
    elif portail == "👤 Mon Espace Enseignant":
        poste_sup = st.checkbox("Poste Supérieur (Décharge 3h)", key="poste_sup_ens_v1")

    if st.button("🚪 Déconnexion", use_container_width=True, key="btn_logout_v1"):
        st.session_state["user_data"] = None
        st.rerun()

# --- APPEL DU MODULE SUIVI D'ASSIDUITÉ (si vous gardez les deux modules) ---
# Si vous souhaitez garder le sélecteur Module 1 / Module 2 du début du fichier,
# laissez votre bloc initial (lignes 124-1867) intact.
# Si vous voulez que tout passe par le nouveau portail ci-dessus, supprimez
# également la première sidebar (lignes 124-1867) et appelez directement :
#
# if portail == "📊 Suivi d'Assiduité":
#     run_assiduite()
