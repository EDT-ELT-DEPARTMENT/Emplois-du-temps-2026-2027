...
def run_assiduite():
    ...
    def rehabiliter_absences_étudiant_supabase(étudiant, matiere):
        if not MODE_SUPABASE:
            return False
        try:
    ...
    def trouver_requete_existante(nom_étudiant, matiere):
        if MODE_SUPABASE:
            try:
    ...
    def supprimer_derniere_absence_supabase(étudiant, matiere, promotion):
        if not MODE_SUPABASE:
            return False
        try:
    ...
    def supprimer_derniere_absence_locale(étudiant, matiere, promotion):
        candidates = [
                ...
                if pwd_admin == CODE_ADMIN:
                    ...
                    if not resultats:
                        ...
                        for req in resultats:
                            with st.expander(f"📄 {req['nom_étudiant']} — {req['matiere']}"):
                                ...
                                st.download_button(
                                    ...
                                    data=pdf_decoded,
                                    file_name=f"Justif_{req['nom_étudiant']}_{req['matiere']}.pdf",
                                ...
                                if col_acc.button("✅ ACCORDER", key=f"acc_{req['id']}", use_container_width=True):
                                    if MODE_SUPABASE:
                                        ...
                                        rehabiliter_absences_étudiant_supabase(req['nom_étudiant'], req['matiere'])
                                    else:
                                        for r in st.session_state.requetes:
                                            if r["id"] == req["id"]:
                                                r["statut"] = "Favorable"
                                    ...
                                    st.success(f"✔️ Justificatif de {req['nom_étudiant']} pour {req['matiere']} accepté.")
                                    time.sleep(0.5)
                                    st.rerun()
